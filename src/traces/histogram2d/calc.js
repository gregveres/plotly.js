...
module.exports = function calc(gd, trace) {
    ...
    if(densitynorm) {
        xinc = makeIncrements(onecol.length, xBinSpec, dx, nonuniformBinsX);
        yinc = makeIncrements(z.length, yBinSpec, dy, nonuniformBinsY);
    }

    // for date axes we need bin bounds to be calcdata. For nonuniform bins
    // we already have this, but uniform with start/end/size they're still strings.
    if(!nonuniformBinsX && xa.type === 'date') xbins = binsToCalc(xr2c, xBinSpec);
    if(!nonuniformBinsY && ya.type === 'date') ybins = binsToCalc(yr2c, yBinSpec);
